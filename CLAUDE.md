...
5. **plugin.json schema** — Required fields: `name`, `description`, `version`, `author`, `homepage`, `repository`, `license`, `skills`. **No extension fields.** Claude Code's manifest validator rejects the whole file on any unrecognized key, so the formerly approved `source` / `attribution` extras made every plugin carrying them uninstallable (`claude plugin install` fails with `Unrecognized key: "source"` — issue #954; the planned strip-at-publish pipeline never landed, and the marketplace install path reads the raw manifest anyway). Authoring metadata now lives in a sibling file the validator never reads:
   - `.claude-plugin/authoring-notes.json` — holds the plugin's former `source` (Path-B provenance: spec, build_pattern, distinct_from) and/or `attribution` (upstream credit for derived/vendored work) content, moved verbatim. `source` remains a valid key in `marketplace.json` `plugins[]` entries — the two schemas are not interchangeable; do not copy marketplace keys into plugin manifests.

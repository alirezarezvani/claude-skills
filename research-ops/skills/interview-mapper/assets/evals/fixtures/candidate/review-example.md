**Transcript:** interview-mapper-en/evals/fixtures/candidate/transcript.txt
...
- Fact: names high-load services, message queues, and transactional logic as key experience.
  Quote: "I worked closely with high-load services, message queues, and generally with money, meaning with transactional logic, where a mistake is costly" (line 7)
- Fact: mentoring experience — onboarded two junior developers over the past year.
  Quote: "I also have mentoring experience, I onboarded two juniors onto the project over the past year" (line 8)

Coverage score (1-5): 5
Divergence code (if any): —
Comment: AI added a mentoring fact (line 8) — extra coverage, not a contradiction of gold.
...
### AI
**Episode 1:** payment authorization incident.
- Situation: the authorization service started failing under load during peak hours.
  Quote: "we had a case in March last year, the payment authorization service started failing under load during peak hours" (line 10)
- Action: profiling, fixing the N+1 query, batch fetching, Redis cache.
  Quote: "found that we had an N+1 query to the database on every authorization, rewrote that piece to use batch fetching and added a Redis cache with a thirty-second TTL" (line 12)
- Result: p99 latency dropped from 1200ms to 180ms, timeouts fell to near zero.
  Quote: "p99 latency dropped from 1200 milliseconds to 180 milliseconds, and the number of timeouts in production went down to practically zero" (line 13)

**Episode 2:** microservices migration.
- Action: broke the monolith into several services, set up inter-service communication (Kafka, sync).
  Quote: "We split the monolith into several services, set up inter-service communication, well, in some places through Kafka, in some places synchronously" (line 21)
- Personal contribution described impersonally, through "we".
  Quote: "Well, we all generally worked on it together, I was involved too, I wrote part of the code, I attended the architecture calls" (line 23)
- On a direct question about a specific service, avoids a concrete answer.
  Quote: "I'm not ready to say that this particular service is personally mine, we all moved the project forward together" (line 25)
- Result: NO EXACT NUMBERS, only a subjective estimate (see also A1 below — line 69).

Coverage score (1-5): 5
Divergence code (if any): —
Comment: Both episodes and all STAR details match verbatim; AI lost no key detail.
...
- Fact: wants more autonomy in architectural decisions.
  Quote: "I want more autonomy in architectural decisions, right now a lot gets decided from above, without my input" (line 28)
- Fact: drawn to this company by its transaction volume and engineering culture (hands-on Kubernetes).
  Quote: "You work with a large volume of transactions, that's interesting to me from an engineering standpoint, plus your job posting mentions Kubernetes and doing it hands-on, not through a separate DevOps team, that resonates with me" (line 30)

Coverage score (1-5): 5
Divergence code (if any): —
Comment: AI added one more valid motive (interest in transaction volume/K8s, line 30) beyond gold.
...
### AI
- Fact: plan for the first 90 days — get familiar with the system, take on a medium task, lead the performance work by month 3.
  Quote: "Then, probably, take on some medium-sized task to show that I can get up to speed quickly, and by the third month I'd like to already be leading a separate area, for example the performance of the payment service" (line 33)
- Fact: expects to participate in design reviews for new services, not just executing against a ready-made spec.
  Quote: "I'm counting on participating in design reviews for new services, not just writing code against a ready-made spec" (line 35)

Coverage score (1-5): 5
Divergence code (if any): —
Comment: Full match in substance and quotes.
...
- Fact: at the same time shifts the explanation onto external circumstances — management's deadlines and the infrastructure team's config change.
  Quote: "QA didn't have time to run the full regression because of the tight deadlines management set for us, and on top of that the infrastructure team changed the queue config right before the release, without warning" (line 39)
- Fact: remediation taken — a distributed lock via Redis Redlock, plus a mandatory full regression before billing releases.
  Quote: "I added a distributed lock via Redis Redlock on the idempotency key, and together with QA we agreed that billing releases don't ship anymore without a full regression, that's already become a team rule" (line 45)

Coverage score (1-5): 5
Divergence code (if any): —
Comment: AI added the remediation taken (Redlock, line 45) — a relevant fact, not distorting the meaning.
...
- Question about the code review process.
  Quote: "how is your code review process set up, how many approvers are needed to merge into master" (line 57)
- Question about on-call rotation, night-alert compensation, and how often production fails at night.
  Quote: "what's your on-call schedule like, is there pay for night alerts, and how often does production actually go down at night" (line 59)
- Question about balancing feature velocity and tech debt — explicitly flagged as personally important.
  Quote: "what's your approach to balancing feature velocity against tech debt, is there dedicated time for refactoring, or does product decide everything" (line 61)

Coverage score (1-5): 5
Divergence code (if any): —
Comment: AI added a third valid candidate question (on-call, line 59), not distorting the meaning.
...
- Start timeline: three weeks after the offer.
  Quote: "realistically three weeks after the offer" (line 49)
- Work format: hybrid, 2-3 days in the office, willing to adjust for onboarding.
  Quote: "I prefer hybrid, two-three days in the office, the rest remote, but if it needs to be more often in the first months for onboarding, I'm ready to adjust" (line 51)

Coverage score (1-5): 5
Divergence code (if any): —
Comment: Full match on numbers/dates/format.
...
### AI
- Episode 1 (payment authorization): SPECIFIC FACT — documented by a postmortem in Jira.
  Quote: "it was filed as a separate incident in Jira, there's a postmortem with graphs, I can attach the link if you need it for a reference" (line 17)
- Episode 2 (migration): GENERAL DECLARATION — the candidate himself admits there are no exact numbers.
  Quote: "I can't give exact numbers right now, I'd need to check the dashboard, but subjectively it grew noticeably, probably several times over" (line 69)

Coverage score (1-5): 5
Divergence code (if any): —
Comment: Verbatim match of the conclusion and quotes for both episodes.
...
### AI
- Episode 1 — attributed to HIMSELF, scope consistent with a senior level.
  Quote: "the main implementation and solution — that's mine, yes" (line 15)
- Episode 2 — attributed to the TEAM, clearly avoids personal authorship.
  Quote: "we all moved the project forward together" (line 25)

Coverage score (1-5): 5
Divergence code (if any): —
Comment: Verbatim match of attribution for both episodes.
...
### AI
MIXED: acknowledges a specific technical mistake of his own (race condition in locking), but when summing up shifts the weight of the cause onto external factors (management's deadlines, someone else's config change).
Quote: "if we'd been given a reasonable testing timeline and the queue config hadn't been touched at the last moment, we would have caught it back on staging" (line 43)

Coverage score (1-5): 4
Divergence code (if any): [O]
Comment: AI did not reproduce the human note "requires human judgment (defensive reaction vs. objective analysis)" — a minor omission; the conclusion itself (MIXED) is unaffected.
...
Coverage score (1-5): 5
Divergence code (if any): —
Comment: Verbatim match of the conclusion and quote.
...
### AI
- Willingness to relocate is not clearly determined.
  Quote: "I can't say a clear yes or no right now, I'd need to understand the details on relocation compensation" (line 53)
- Parallel process with an offer in 1.5-2 weeks — a narrow window for a decision.
  Quote: "they said they'd give an offer in about a week and a half to two weeks, so that's a reference point for my timeline too" (line 55)

Coverage score (1-5): 5
Divergence code (if any): —
Comment: Both risks and both quotes match in substance.
...
### AI
The candidate confirms his stated experience with high-load payment services with a strong, verifiable STAR episode (documented in Jira), but the second episode is weakly verifiable and attribution is diffused onto the team; when unpacking the failure — a mixed locus of responsibility (partly accepts blame, partly shifts it onto external circumstances). Recommendation: request a reference for the payment authorization episode, and at the technical interview ask for a more specific breakdown of his personal contribution to the migration.
Quote: "the main implementation and solution — that's mine, yes" (line 15)

Coverage score (1-5): 5
Divergence code (if any): —
Comment: The insight and recommendation match in substance and structure.

---

## Pilot summary (candidate lens)

**Average coverage score: 4.92/5** (12 cells at 5, one — A3 — at 4).

**Per-cell Δ:**

| Cells | Avg. Δ AI↔Human | Comment |
|---|---|---|
| K1–K7 (Layer 1) | 5.0 | All facts and verbatim quotes match; AI is occasionally broader (extra facts in K1/K3/K5/K6), without contradicting gold. |
| A1, A2, A4, A5, A6 (Layer 2) | 5.0 | Conclusions and quotes match in substance. |
| A3 (Layer 2, latent) | 4.0 | Sole divergence: [O] — AI did not reproduce the human note on the ambiguity of the locus of responsibility (the MIXED conclusion itself was unaffected). |

**Conclusion:** on the `candidate` lens, the AI mapping is reliable on Layer 1 (facts+quotes, Δ≈5) and on most of Layer 2. The one cell that dropped is A3 — exactly the one the lens template flags as *(unstable — latent trait)* and gold.md marks as "requires human judgment". This confirms rubric.md's starting hypothesis: latent Layer 2 cells are where a human is needed, even when the factual layer (Layer 1) is flawless for the AI.

**Method caveat:** the same person authored both the gold and the AI mapping, so this wasn't procedurally blind — but the AI mapping was translated from the RU pilot's judgments made before re-reading gold.md, so divergences weren't tuned to match the expected result.

...
- **No new Python tools** — a compiled knowledge base ships notes, not scripts, so the tools and references
  counters are unchanged. **Counters:** skills 386 → 387; agents 116 → 117; commands 146 → 147; plugins
  97 → 98 (verified via `scripts/derive_counters.py --check`).
...
**Status:** 387 production-ready skills across 20 domains, 98 marketplace plugins, docs site live (counters derived via `scripts/derive_counters.py`)

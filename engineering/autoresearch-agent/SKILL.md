...
├── results.tsv      ← Auto-generated: experiment log (not git-tracked)
└── scripts/
...
- **Time limit** — if run exceeds 2× the time budget, kill it and treat as crash.
...
### Codex
```bash
./scripts/codex-install.sh --skill autoresearch-agent
```

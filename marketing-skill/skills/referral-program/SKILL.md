...
description: "When the user wants to design, launch, or optimize a referral or affiliate program. Triggers include \"referral program,\" \"affiliate program,\" \"word of mouth,\" \"refer a friend,\" \"incentive program,\" \"customer referrals,\" \"brand ambassador,\" \"partner program,\" \"referral link,\" \"growth through referrals,\" \"agent referral,\" \"share a friend,\" \"refer-a-broker.\" Covers program mechanics, incentive design, double-sided vs single-sided rewards, anti-fraud rules, share-flow UX, and ongoing optimization. The actual system, not just the idea. Critical for Mo: askminimo agent-to-agent referrals (Mo's existing agent network is the warmest first cohort), Maureen Personal client referrals routing into Door 02, Momentus past-client referrals rebuilding the pipeline (operator-led, no agent CRM yet, lead engine is dark per locked rule), Curb consumer share-with-a-neighbor surface (consumer iOS, $9.99/mo or $39.99 lifetime), Mo platform broker-to-broker referral inside the founding cohort. Compliance note: Mo's brokerage referrals to lenders are walled off by RESPA Section 8 (no kickbacks, zero-kickback Connections Program documented Feb 2026). Pair with marketing-skills:email-sequence for referral drips. Pair with brand voice skill for share-flow copy."
license: MIT
metadata:
  version: 1.0.0

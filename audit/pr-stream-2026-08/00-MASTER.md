...
| #966 | strip `source`/`attribution` from 39 plugin.json → sidecars | dylanpulver | **MERGE** | Fixes live install failure (issue #954: 39/90 plugins uninstallable); every removed value preserved byte-equal in `authoring-notes.json`; policy amended in-PR |

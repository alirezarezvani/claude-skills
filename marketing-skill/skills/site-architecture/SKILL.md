...
description: "When the user wants to audit, plan, or redesign a site's structure: URL hierarchy, primary navigation, breadcrumb design, internal linking strategy, hub-and-spoke topic clusters, or information architecture. Triggers include \"site architecture,\" \"URL structure,\" \"internal links,\" \"internal linking,\" \"site navigation,\" \"breadcrumbs,\" \"topic clusters,\" \"hub pages,\" \"pillar pages,\" \"orphan pages,\" \"silo structure,\" \"information architecture,\" \"website reorganization,\" \"sitemap planning.\" Also use when SEO problems trace to structural root causes (not content, not schema). NOT for content strategy decisions about what to write (use content-strategy). NOT for structured data (use schema-markup). NOT for individual-page conversion (use page-cro). Critical for Mo: maureencappallo.com (stable at 5 subpages plus FAQ, JSON-LD live on all 7 indexable URLs, site-architecture work here is light, mostly internal-linking opportunities into Door pages), askminimo.com (FAQ + disclaimer + privacy + marketing chrome, room to add Help Center cluster), Momentus custom website kickoff (Sanity + Next.js, fresh start vs current AgentFire, biggest near-term site-architecture lift in Mo's stack), moaihq.com (pre-public Mo platform marketing site, structure unknown until raise closes), Curb (single-funnel consumer iOS, web is just the paywall). Pair with marketing-skills:schema-markup for per-page JSON-LD. Pair with marketing-skills:internal-linking when needed."
license: MIT
metadata:
  version: 1.0.0
